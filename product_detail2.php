...
$product_images = ['server-ring.png','server-ring-4.png','ring-server-1.png'];
if (!empty($product['image'])) $product_images[] = trim($product['image']);
if (!empty($product['images'])) {
    $extra = array_filter(array_map('trim', explode(",", $product['images'])));
    $product_images = array_merge($product_images, $extra);
}

// Ensure at least one image exists
if (empty($product_images)) $product_images[] = 'placeholder.jpg'; // fallback image
